...
using immer::archive::container_id;
using immer::archive::rbts::save_to_archive;
...
{
    const auto archive =
        test::from_json<immer::archive::rbts::archive_load<int>>(json);
    auto loader =
        immer::archive::rbts::make_loader_for(test::example_vector{}, archive);
...
auto load_flex_vec(const auto& json, std::size_t vec_id)
{
    const auto archive =
        test::from_json<immer::archive::rbts::archive_load<int>>(json);
    auto loader = immer::archive::rbts::make_loader_for(
...
TEST_CASE("Save and load multiple times into the same archive")
{
    ...
    auto ar = immer::archive::rbts::make_save_archive_for(example_vector{});
...
TEST_CASE("Save and load vectors with shared nodes")
{
    ...
    const auto save_vectors = [](const auto& vectors)
        -> std::pair<example_archive_save,
                     std::vector<immer::archive::container_id>> {
        auto ar  = example_archive_save{};
        auto ids = std::vector<immer::archive::container_id>{};
...
TEST_CASE("Save and load vectors and flex vectors with shared nodes")
{
    ...
    auto vector_ids          = std::vector<immer::archive::container_id>{};
    auto flex_vectors_ids    = std::vector<immer::archive::container_id>{};
...
TEST_CASE("A loop with 2 nodes")
{
    ...
    REQUIRE_THROWS_AS(load_flex_vec(json, 0),
                      immer::archive::archive_has_cycles);
}
...
TEST_CASE("Test vector with very big objects")
{
    ...
    auto ar = immer::archive::rbts::make_save_archive_for(
...
TEST_CASE("Test modifying vector nodes")
{
    ...
    SECTION("Load non-existing vector")
    {
        REQUIRE_THROWS_AS(load_vec(data.dump(), 99),
                          immer::archive::archive_exception);
    }
    ...
    SECTION("A leaf with too few elements")
    {
        ...
        item["value"] = {2};
        REQUIRE_THROWS_AS(load_vec(data.dump(), 0),
                          immer::archive::rbts::vector_corrupted_exception);
    }
    ...
    SECTION("Mess with the tail")
    {
        ...
        SECTION("Add too many elements")
        {
            ...
            item["value"] = {6, 7, 8};
            REQUIRE_THROWS_AS(load_vec(data.dump(), 0),
                              immer::archive::invalid_children_count);
        }
    }
    ...
    SECTION("Modify children")
    {
        ...
        SECTION("Strict vector can not have relaxed nodes")
        {
            ...
            REQUIRE_THROWS_AS(
                ...
                immer::archive::rbts::relaxed_node_not_allowed_exception);
        }
    }
}
    ...
    SECTION("Modify inner node")
    {
        ...
        SECTION("Mix leaves and inners as children")
        {
            ...
            REQUIRE_THROWS_AS(
                ...
                immer::archive::rbts::same_depth_children_exception);
        }
        SECTION("No children")
        {
...
TEST_CASE("Test flex vector with a weird shape relaxed")
{
    ...
    // {
    //     auto ar        = immer::archive::rbts::make_save_archive_for(loaded);
    //     auto vector_id = immer::archive::node_id{};
    //     std::tie(ar, vector_id) =
    //         immer::archive::rbts::save_to_archive(loaded, ar);
...
TEST_CASE("Flex vector converted from strict")
{
    ...
    auto ar           = immer::archive::rbts::make_save_archive_for(small_vec);
    auto small_vec_id = immer::archive::container_id{};
    auto small_flex_vec_id = immer::archive::container_id{};
...
TEST_CASE("Can't load saved flex vector with relaxed nodes as strict")
{
    ...
    auto vec_id          = immer::archive::container_id{};
...
TEST_CASE("Test vector archive conversion")
{
    ...
    auto vec2_id          = immer::archive::container_id{};
    ...
    const auto expected_ar = json_t::parse(R"(
{
  "value0": {
    ...
    {
        auto ar = immer::archive::rbts::make_save_archive_for(loaded_1);
        auto id = immer::archive::container_id{};
