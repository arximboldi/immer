...
{
    auto str = std::string{};
    REQUIRE(immer::archive::xx_hash<std::string>{}(str) == 3244421341483603138);
    REQUIRE(XXH3_64bits(str.c_str(), str.size()) == 3244421341483603138);

    str = "hello";
    REQUIRE(immer::archive::xx_hash<std::string>{}(str) ==
...
TEST_CASE("Test loading a big map saved on macOS with std::hash", "[.macos]")
{
  ...
  "value0": [
    ...
    const auto loaded_archive = test::from_json<
        immer::archive::champ::container_archive_load<Container>>(ar_str);
...
TEST_CASE("Test loading a big map with xxHash")
{
    using Container = immer::
        map<std::string, std::string, immer::archive::xx_hash<std::string>>;
