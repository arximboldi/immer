...
TEST_CASE("Test vector pool conversion")
{
    using test::new_type;
    using test::old_type;
    ...
    auto pool               = immer::persist::rbts::make_output_pool_for(vec1);
    auto vec1_id            = immer::persist::container_id{};
    std::tie(pool, vec1_id) = add_to_pool(vec1, pool);
    auto vec2_id            = immer::persist::container_id{};
    std::tie(pool, vec2_id) = add_to_pool(vec2, pool);

    // Confirm that vec1 and vec2 have structural sharing in the beginning.
    const auto expected_pool = json_t::parse(R"(
{
  "value0": {
    "inners": [
      {
    ...
    REQUIRE(json_t::parse(to_json(pool)) == expected_pool);

    const auto in_pool          = to_input_pool(pool);
    const auto in_pool_new_type = transform_pool(in_pool, convert_old_type);
    auto loader =
        make_loader_for(test::vector_one<new_type>{}, in_pool_new_type);
    ...
    {
        auto pool = immer::persist::rbts::make_output_pool_for(loaded_1);
        auto id   = immer::persist::container_id{};
